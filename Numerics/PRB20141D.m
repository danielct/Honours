...
%Coefficients in the ODGPE: (normalised for  m_LP=10^{-3} me)
 ...
 R=0.1; %stimulated scattering rate
...
%Pumping
Pump=2.0 * Pth * (exp(-0.005 *((x).^2 +(y).^2))); %Gaussian pump
% Pump=40*(exp(-0.0005 *((x).^2+(y).^2))); %Gaussian pump
...
for step = 1:300
    ...
    % exp_factor_polaritons=exp((-1i*potential-1i*g*psi1.*conj(psi1)-1i*gR*n1+0.5*(R*n1-GammaC))*dt);
